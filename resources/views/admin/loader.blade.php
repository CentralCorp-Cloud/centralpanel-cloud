...
@section('content')
        ...
        <div class="card shadow-sm border-0">
            <div class="card-body">
                <form method="POST" action="{{ route('admin.loader.update') }}">
                    ...
                    <div class="mb-3">
                        <label for="minecraft_version"
                            class="form-label fw-semibold">{{ __('messages.loader.minecraft_version') }}</label>
                    ...
                    <div class="form-check form-switch mb-3">
                        ...
                        <input type="checkbox" class="form-check-input" id="loader-activation" name="loader_activation"
                            value="1" {{ ($row->loader_activation ?? false) ? 'checked' : '' }}>
                        <label class="form-check-label"
                            for="loader-activation">{{ __('messages.loader.enable_loader') }}</label>
                    </div>

                    <div class="mb-3">
                        <label for="loader-type"
                            class="form-label fw-semibold">{{ __('messages.loader.loader_type') }}</label>
                    ...
                    <div class="mb-4">
                        <label for="loader-build-version"
                            class="form-label fw-semibold">{{ __('messages.loader.build_version') }}</label>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            // Sync the build select dropdown value into the hidden loader_build_version input
            ...
            // Also sync on form submit to ensure the value is always saved
            const form = buildSelect.closest('form');
